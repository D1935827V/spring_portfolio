package com.example.sping_portfolio.Ascii;

import java.awt.*;

public class AsciiArt {
    // The lookup table for all the chars that can be used
    ...
    public static char brightnessToChar(double brightness) {
        int i = 0;
        while (brightness > GRAYSCALE_VALUES[i]) {
            i++;
        }
    ...
    public static Object main(Picture args) {
        Picture picture = new Picture("src/main/resources/static/images/dnhsschool.png");
...
// public static <converts> char[][] main(Picture picture) {
//       double[][] image = getGrayscaleArray(picture);
